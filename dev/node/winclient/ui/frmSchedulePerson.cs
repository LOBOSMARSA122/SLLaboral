    ...
    public partial class frmSchedulePerson : Form
    {
        ...
        private void btnFilter_Click(object sender, EventArgs e)
        {
            ...
            List<ServiceList>  objServiceList = new List<ServiceList>(); 
                ...
                if (objpacientDto != null)
                {
                    ...
                    objServiceList = objServiceBL.GetServicesPagedAndFiltered(ref objOperationResult, null, null, "", "v_PersonId=" + "\"" + PacientId + "\"",null,null, null);
                ...
                if (uvschedule.Validate(true, false).IsValid)
                {
                    ...
                    if (ModeAgenda == "New")
                    {
                        using (new LoadingClass.PleaseWait(this.Location, "Grabando..."))
                        {
                            ...
                            if (ddlMasterServiceId.SelectedValue.ToString() == ((int)MasterService.AtxMedicaParticular).ToString())
                            {
                                CalendarId = _objCalendarBL.AddShedule(ref objOperationResult, objCalendarDto, Globals.ClientSession.GetAsList(), Constants.CONSULTAMEDICA, PacientId, Int32.Parse(ddlMasterServiceId.SelectedValue.ToString()), NuevoContinuacion);
                            }
                            else
                            {
        ...
        private void grdDataService_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (row != null)
                {
                    int i_MasterServiceId = int.Parse(grdDataService.Selected.Rows[0].Cells[4].Value.ToString());

                    if (i_MasterServiceId == (int)Common.MasterService.Eso)
                    {
                        _ProtocolId = grdDataService.Selected.Rows[0].Cells[2].Value.ToString();
                        _CalendarId = grdDataService.Selected.Rows[0].Cells[1].Value.ToString();
                        objProtocol = _objProtocoltBL.GetProtocolById(ref objOperationResult, _ProtocolId);
