                    ...
                    if (Ext == ".XLSX" || Ext == ".XLS")
                    {
                        ...
                        while (worksheet1.Rows[i].Cells[0].Value != null )
                        {
                            ...
                            //Fecha Nacimiento
                             ...
                             {
                                 TempPacient.v_CurrentOccupation = worksheet1.Rows[i].Cells[10].Value.ToString();
                                 Imported = true;
                             }
                             else
                             {
            ...
            if (uvschedule.Validate(true, false).IsValid)
            {
                    foreach (var item in _TempPacientList)
                    {
                        ...
                        if (PacientId != null && Verificar == null)  // Se grabo el paciente y se lo agenda
                        {
                            ...
                            CalendarId = objCalendarBL.AddShedule(ref objOperationResult, objCalendarDto, Globals.ClientSession.GetAsList(), _ProtocolId, PacientId, Int32.Parse(ddlMasterServiceId.SelectedValue.ToString()),"Nuevo");
