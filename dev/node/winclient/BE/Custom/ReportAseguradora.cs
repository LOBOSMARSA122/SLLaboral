using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Sigesoft.Node.WinClient.BE
{
    public class ReportAseguradora
    {
        public string IdServicio { get; set; }
        public string IdProtocolId { get; set; }
        public string IdTrabajador { get; set; }
        public string Trabajador { get; set; }
        public string Parentesco { get; set; }
        public string NombreTitular { get; set; }
        public string EmpresaTrabajo { get; set; }
        public string Producto { get; set; }
        public string DireccionTrabajador { get; set; }
        public string NroDNI { get; set; }
        public string PuestoTrabajo { get; set; }
        public string Proveedor { get; set; }
        public string LugarChekeo { get; set; }
        public string Modalidad { get; set; }
        public DateTime FechaServicioDate { get; set; }
        public string FechaServicio { get; set; }
        public string TelefonoTrabajador { get; set; }
        public string CelularTrabajador { get; set; }
        public string EmailTrabajador { get; set; }
        public string MedicoEvaluador { get; set; }
        public DateTime? FechaNacimiento { get; set; }
        public int EdadTrabajador { get; set; }
        public string GrupoEtario { get; set; }
        public string GeneroTrabajador { get; set; }
        public string AntecedentePersonal1 { get; set; }
        public string AntecedentePersonal2 { get; set; }
        public string AntecedentePersonal3 { get; set; }
        public string AntecedentePersonal4 { get; set; }
        public string AntecedentePersonal5 { get; set; }
        public string AntecedentePersonal6 { get; set; }
        public string Anannesis { get; set; }
        public string ActividadFisica { get; set; }
        public string Fumar { get; set; }
        public string Alcohol { get; set; }
        public string Drogas { get; set; }
        public string AntecedenteFamiliar1 { get; set; }
        public string AntecedenteFamiliar2 { get; set; }
        public string AntecedenteFamiliar3 { get; set; }
        public string AntecedenteFamiliar4 { get; set; }
        public string AntecedenteFamiliar5 { get; set; }
        public string PresionSistolica { get; set; }
        public string PresionDiastolica { get; set; }
        public string PresionArterialDx { get; set; }
        public string PerimetroAbdominal { get; set; }
        public string Talla { get; set;}
        public string Peso { get; set; }
        public string IMC { get; set; }
        public string IMCDx { get; set; }
        public string PesoMaximo { get; set; }

        public string Hemograma { get; set; }
        public string HemogramaA { get; set; }

        public string HemoglobinaA { get; set; }
        public string Hemoglobina { get; set; }       
        public string HemoglobinaB { get; set; }
        public string HemoglobinaDX { get; set; }
       
        public string GlucosaA { get; set; }
        public string Glucosa { get; set; }
        public string GlucosaB { get; set; }
        public string GlucosaDx { get; set; }
       
        public string ColesterolA { get; set; }
        public string Colesterol { get; set; }
        public string ColesterolB { get; set; }
        public string ColesterolDx { get; set; }

        public string HDLColesterolA { get; set; }
        public string HDLColesterol { get; set; }
        public string HDLColesterolB { get; set; }
        public string HDLDx { get; set; }

        public string TrigliceridoA { get; set; }
        public string Triglicerido { get; set; }
        public string TrigliceridoB { get; set; }
        public string TrigliceridoDx { get; set; }

        public string PSAA { get; set; }
        public string PSA { get; set; }
        public string PSAB { get; set; }
        public string PSADx { get; set; }

        public string ExamenFisicoDescripcion { get; set; }
        public string ExamenClinico2 { get; set; }

        public string ExamenOrinaA { get; set; }
        public string ExamenOrina { get; set; }
        public string THENEVON { get; set; }

        public string Rx { get; set; }
        public string RxDescripcion { get; set; }

        public string ElectrocardiogramaA { get; set; }
        public string Electrocardiograma { get; set; }

        public string EvaluacionGinecologica { get; set; }
        public string Papanicolao { get; set; }
        public string Mamografia { get; set; }
        public string EcoPelvica { get; set; }
        public string EcoTransvaginal { get; set; }
        public string Colonoscopia { get; set; }
        public string DesintometriaOsea { get; set; }
        public string EvOdontologica { get; set; }
        public string EvOftalmologica { get; set; }
        public string EvPsicológica { get; set; }

        public string Dx1 { get; set; }
        public string Dx2 { get; set; }
        public string Dx3 { get; set; }
        public string Dx4 { get; set; }
        public string Dx5 { get; set; }
        public string Dx6 { get; set; }
        public string Dx7 { get; set; }
        public string Dx8 { get; set; }

        public string Recomendacion1 { get; set; }
        public string Recomendacion2 { get; set; }
        public string Recomendacion3 { get; set; }
        public string Recomendacion4 { get; set; }
        public string Recomendacion5 { get; set; }
        public string Recomendacion6 { get; set; }
        public string Recomendacion7 { get; set; }
        public string Recomendacion8 { get; set; }
        public string Recomendacion9 { get; set; }
        public string Recomendacion10 { get; set; }
        public string Recomendacion11 { get; set; }
        public string Recomendacion12 { get; set; }
        public string Recomendacion13 { get; set; }
        public string Recomendacion14 { get; set; }

        public string v_CustomerOrganizationName { get; set; }
        public string v_CustomerOrganizationId { get; set; }
        public string v_CustomerLocationId { get; set; }
        public string Aptitud { get; set; }
        public string DxAudiometria { get; set; }
        public string DxEspirometria { get; set; }

        public string restricciones { get; set; }

        public string GrupoFactorSanguineo { get; set; }
        public string TestAltura { get; set; }
        public string TestSomnolencia { get; set; }
        
    }
}
