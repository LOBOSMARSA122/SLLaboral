...
namespace Sigesoft.Node.WinClient.UI.Reports
{
    public partial class frmConsolidatedReports : Form
    {
        ...
        private DataSet GetReportCertificadoAptitud()
        {
            ...
            //var Conclusiones = new ServiceBL().GetAptitudeCertificateConclusiones(ref objOperationResult, _serviceId);
            ...
            //DataTable dt1 = Sigesoft.Node.WinClient.BLL.Utils.ConvertToDatatable(Conclusiones);
            //dt1.TableName = "dtConclusiones";
            //dsGetRepo.Tables.Add(dt1);
        ...
        private DataSet GetTestVertigo()
        {
            ...
            var dataListForReport = new ServiceBL().GetReportTestVertigo(_serviceId, Constants.TEST_SINTOMATICO_RESP_ID);
        ...
        private DataSet GetTestSueño()
        {
            OperationResult objOperationResult = new OperationResult();

            var dataListForReport = new ServiceBL().GetReportTestSueño(_serviceId, Constants.TEST_CHOFERES_ID);

            dsGetRepo = new DataSet();

            DataTable dt = Sigesoft.Node.WinClient.BLL.Utils.ConvertToDatatable(dataListForReport);
            dt.TableName = "dtTestChoferes";
            dsGetRepo.Tables.Add(dt);

            return dsGetRepo;
        }

        private DataSet GetTestSomnolencia()
        {
            OperationResult objOperationResult = new OperationResult();

            var dataListForReport = new ServiceBL().GetReportTestSomnolencia(_serviceId, Constants.TEST_SOMNOLENCIA_ID);

            dsGetRepo = new DataSet();

            DataTable dt = Sigesoft.Node.WinClient.BLL.Utils.ConvertToDatatable(dataListForReport);
            dt.TableName = "dtTestSomnolencia";
            dsGetRepo.Tables.Add(dt);
        ...
        private DataSet GetTestSintomaticoResp()
        {
            OperationResult objOperationResult = new OperationResult();

            var dataListForReport = new ServiceBL().GetReportTestSintomaticoResp(_serviceId, Constants.TEST_SINTOMATICO_RESP_ID);

            dsGetRepo = new DataSet();

            DataTable dt = Sigesoft.Node.WinClient.BLL.Utils.ConvertToDatatable(dataListForReport);
            dt.TableName = "dtTestSintomaticoResp";
            dsGetRepo.Tables.Add(dt);
        ...
        private void ChooseReport(crConsolidatedReports rp, string componentId)
        {
            ...
            switch (componentId)
            {
                ...
                case Constants.TEST_CHOFERES_ID:
                    // Osteomuscular1
                    ds = GetTestSueño();
                    rp.Subreports["crTestDeSueño.rpt"].SetDataSource(ds);
                    rp.DetailSection16.SectionFormat.EnableSuppress = false;
                    break;

                case Constants.TEST_SOMNOLENCIA_ID:
                    // Osteomuscular1
                    ds = GetTestSomnolencia();
                    rp.Subreports["crTestEpwotrh.rpt"].SetDataSource(ds);
                    rp.DetailSection17.SectionFormat.EnableSuppress = false;
                    break;
                case Constants.INFORME_CERTIFICADO_APTITUD_OBSERVADO:
                    ds = GetReportCertificadoAptitud();

                    rp.Subreports["cr.CertficadoObservado.rpt"].SetDataSource(ds);
                    rp.DetailSection18.SectionFormat.EnableSuppress = false;                   
                    break;
                case Constants.TEST_SINTOMATICO_RESP_ID:
                    ds = GetTestSintomaticoResp();
                    rp.Subreports["cr.TestSintomaticoResp.rpt"].SetDataSource(ds);
