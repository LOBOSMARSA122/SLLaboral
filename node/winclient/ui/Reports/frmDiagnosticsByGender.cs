...
{
    public partial class frmDiagnosticsByGender : Form
    {
        ServiceBL _serviceBL = new ServiceBL();
        string strFilterExpression;
        string _IdEmpresaClienete;
        public frmDiagnosticsByGender()
        {
            InitializeComponent();
        }

        private void frmDiagnosticsByGender_Load(object sender, EventArgs e)
        {
        ...
        private void btnFilter_Click(object sender, EventArgs e)
        {
            if (uvReporte.Validate(true, false).IsValid)
            {
                ...
                if (cbGenero.SelectedValue.ToString() != "-1")
                    Filters.Add("i_SexTypeId==" + int.Parse(cbGenero.SelectedValue.ToString()));
        ...
        private void ShowReport(DateTime? beginDate, DateTime? endDate)
        {
            ...
            var dataList = _serviceBL.ReportDiagnosticsByGender(beginDate, endDate, strFilterExpression, int.Parse(cbTop.Text));
            


            var Diagnosticos = _serviceBL.ReportDiagnosticsByGenderDetallado(beginDate, endDate, strFilterExpression, int.Parse(cbTop.Text));
            ...
            crvDiagnosticsByGender.ReportSource = rp;
            crvDiagnosticsByGender.Show();
        }
